...
Maybe<SimpleTurnMessage> SimpleTurnMessage::deserialize(uint8_t* msg,
    ...
    TurnDirection direction = static_cast<TurnDirection>((msg[0] & DIRECTION_MASK) >> DIRECTION_OFFSET);

    return {SimpleTurnMessage(speed, direction)};
}

SimpleTurnMessage::SimpleTurnMessage(const TurnSpeed& speed,
        const TurnDirection& direction) : speed(speed), direction(direction) {
}
...
int SimpleTurnMessage::serialize(uint8_t* buffer, int buffersize) {
    ...
    payload |= (static_cast<uint8_t>(direction) << DIRECTION_OFFSET) & DIRECTION_MASK;
...
void SimpleTurnMessage::print() {
    printf("SimpleTurnCommand[speed=%s, direction=%s]", enumToString(speed), enumToString(direction));
}
