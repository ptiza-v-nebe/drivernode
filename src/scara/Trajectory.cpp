...
XTrajectory Trajectory::buildJointspace() {
...
void Trajectory::addPose(TimeFactors dTimeFactor, Pose pose) {
	...
	dFactors[k] = dTimeFactorNum / 100;
	k++;

	double sumOfDFactors = std::accumulate(dFactors.begin(), dFactors.end(), 0);

	std::vector<double> newDFactors;
	std::for_each(dFactors.begin(), dFactors.end(), [&](const double& d) {
		newDFactors.push_back(d / sumOfDFactors);
	});
	...
	//copy poses
	std::copy(trj[i].begin() + 1, trj[i].end(), pose.begin());

	//copy time and poses to vectors
	std::for_each(trj.begin(), trj.end(), [&](std::vector<double> & row) {
		time.push_back(row[0]);
		data.push_back( {row.begin()+1,row.end()});
	});
}
...
std::vector<std::vector<double>> Trajectory::interpolate(
	...
	for (unsigned int i = 0; i < interTimes.size(); i++) {
		interTrajectory[i] = {interTimes[i],interPoseX[i],interPoseY[i],interPoseZ[i],interPoseT[i]};
	}
...
//void Trajectory::matrixMul(std::std::vector<std::vector<double>> a, std::vector<std::vector<double>> b){
//
////	for (i = 0; i < N; i++) {
////			for (j = 0; j < N; j++) {
////				c[i][j] = 0.0;
...
std::vector<double> Trajectory::getTimeValues() {
}

std::vector<double> Trajectory::getXValues() {
}

std::vector<double> Trajectory::getYValues() {
}

std::vector<double> Trajectory::getZValues() {
}

std::vector<double> Trajectory::getThetaValues() {
}

std::vector<double> Trajectory::getPumpValues() {
}
